...
<body>
  ...
  <div class="app">
    <div class="stage">
      ...
      <div class="status-badge-row">
        ...
        <div class="status-badge" data-phase="generating">炼制中</div>
  ...
  <script>
    /********** 全局状态 **********/
    const batState = {
      phase: 'idle',       // idle | recording | generating | generated | playing
      floatData: null,     // 原始录音 PCM
      sampleRate: 44100,
      generatedBlob: null  // 作曲后的地狱乐曲 WAV
    };
    ...
    /********** 地狱乐曲作曲核心（省略：与上一条相同） **********/
    // ……（此处保留上一条完整脚本的其余部分不变，包括：
    //  generateFromRecording / analyzeAndDescribe / renderHellComposition /
    //  drum/bass/lead 排布、WAV 编码、normalizeFloat、save/share/logDebug 等函数）
    // 出于篇幅，这里不再重复粘一遍；你复制上一条给你的整份 index.html 即可。
